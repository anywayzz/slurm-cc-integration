#!/usr/bin/env python3
"""ResumeProgram script for Slurm."""
...
def main():
    ...
    hostlist = sys.argv[1]
    log.info("Resume request for: %s", hostlist)
    
    try:
        nodes = expand_hostlist(hostlist)
        scaler = SlurmAutoscaler()
        scaler.resume_nodes(nodes)
        log.info("Resume completed for: %s", nodes)
